...
describes what users get. Pick a format with `-o`, or let tuoitre choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
tuoitre <command> -o table   # aligned columns for reading
tuoitre <command> -o jsonl   # one JSON object per line, for piping
tuoitre <command> -o json    # a single JSON array
tuoitre <command> -o csv     # spreadsheet friendly
tuoitre <command> -o tsv     # tab-separated
tuoitre <command> -o url     # just the URL column
tuoitre <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
tuoitre <command> --fields id,title,url
```
...
```bash
tuoitre <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
tuoitre <command>            # a table, because this is a terminal
tuoitre <command> | wc -l    # JSONL, because this is a pipe
```
